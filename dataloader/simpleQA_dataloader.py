...
class SimpleQADataset(Dataset):
    ...
    def freq_of_relations(self,):
        threshold_values = 0.6 * max(self.label_freq.values())
        rare_labels = []
        pop_labels = []
        for i in range(len(self.vocab.rtoi)):
            if i in self.label_freq and self.label_freq[i] > threshold_values:
                pop_labels.append(i)
            else:
                rare_labels.append(i)
        return np.array(rare_labels),np.array(pop_labels)
